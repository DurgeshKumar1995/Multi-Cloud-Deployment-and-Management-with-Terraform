...
Route 53 creates one provider hostname per enabled cloud and a weighted record set for `app.multicloud.durgesh.space`. Each record is associated with an HTTP `/health` check. Equal weights spread DNS responses; unhealthy endpoints are removed from responses. DNS failover is not instantaneous because recursive resolvers cache results.
